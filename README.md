...
```cmake
find_package(nanofix 1.0 CONFIG REQUIRED)   # CMAKE_PREFIX_PATH=/opt/nanofix
target_link_libraries(my_app PRIVATE nanofix::nanofix)
```
...
> **macOS 26.5, Apple M4, AppleClang 21, unpinned — indicative only.** macOS
> has no hard core pinning and Apple Silicon migrates work across P/E cores
> mid-run, so these drift run-to-run. Read them as rough shape, not
> measurements — authoritative numbers come from a pinned, isolated Linux core
> (`NANOFIX_BENCH_CPU=<cpu> compare2upstream/run.sh`; governor `performance`,
> `isolcpus`). `nanofix` is `fork`; `fork-no-simd` is the same code with
> `NANOFIX_DISABLE_SIMD`; upstream is `jamesdbrock/hffix`. Generated 2026-07 by
> `compare2upstream/run.sh` (`min_time=0.3s`, `repetitions=10`; cells are means
> over repetitions).
...
| --- | --- | --- | --- |
| `Write p99` | 125 ns | **84 ns (-33 %)** | 96.4 ns (-23 %) |
| `Write p999` | 145.8 ns | **100.4 ns (-31 %)** | 112.9 ns (-23 %) |
| `Read seq p99` | 86079 ns | **60762 ns (-29 %)** | 61404 ns (-29 %) |
| `Read seq p999` | 104200 ns | **76183 ns (-27 %)** | 81471 ns (-22 %) |
| `Read rand p99` | 124846 ns | **88458 ns (-29 %)** | 91929 ns (-26 %) |
| `Read rand p999` | 147229 ns | **101829 ns (-31 %)** | 125279 ns (-15 %) |
...
| Benchmark | upstream | fork | fork-no-simd |
| --- | --- | --- | --- |
| `Write (NewOrder)` | 13.5 M msgs/s | 19.8 M msgs/s (+47 %) | **20.2 M msgs/s (+49 %)** |
| `Read seq` | 31.3 k msgs/s | **44.3 k msgs/s (+42 %)** | 43.8 k msgs/s (+40 %) |
| `Read random` | 21.5 k msgs/s | **29.7 k msgs/s (+38 %)** | 29.5 k msgs/s (+37 %) |
...
| --- | --- | --- | --- |
| `Read seq p99` | 86079 ns | **4717 ns (-95 %)** | 12017 ns (-86 %) |
| `Read seq p999` | 104200 ns | **6129 ns (-94 %)** | 15854 ns (-85 %) |
| `Read rand p99` | 124846 ns | **4992 ns (-96 %)** | 15150 ns (-88 %) |
| `Read rand p999` | 147229 ns | **6046 ns (-96 %)** | 22854 ns (-84 %) |
...
| Benchmark | upstream iterator | fork indexed | fork-no-simd indexed |
| --- | --- | --- | --- |
| `Read seq` | 31.3 k msgs/s | **532 k msgs/s (+1602 %)** | 215 k msgs/s (+588 %) |
| `Read random` | 21.5 k msgs/s | **504 k msgs/s (+2246 %)** | 192 k msgs/s (+791 %) |
