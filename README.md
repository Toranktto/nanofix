...
`run.sh` writes the rendered tables to an arch-named file next to itself —
that file is the committed snapshot, regenerated by re-running the harness:

- **[compare2upstream/ARM64.md](compare2upstream/ARM64.md)** — Apple M4,
  macOS, unpinned: **indicative only** (no hard core pinning on macOS, P/E
  migration mid-run).
- **[compare2upstream/X64.md](compare2upstream/X64.md)** — placeholder until
